...
(function ($)
{
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            {
            	//修改,如果大于窗口高度往上
				//alert(g.text.offset().top + 1 + g.text.outerHeight() + g.dateeditor.height());
				//alert($(window).height());
				if ((g.text.offset().top + 1 + g.text.outerHeight() + g.dateeditor.height()) > $(window).height()) {
					g.dateeditor.css({ left: g.text.offset().left, top: g.text.offset().top + 1 - g.dateeditor.height() - 4});
				} else {
