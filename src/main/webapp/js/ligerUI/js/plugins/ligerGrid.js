...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
		...
		pageParmName: 'param.currentPage',               //页索引参数名，(提交给服务器)
        pagesizeParmName: 'param.pageSize',        //页记录数参数名，(提交给服务器)
        sortnameParmName: 'param.sortName',        //页排序列名(提交给服务器)
        sortorderParmName: 'param.sortOrder',      //页排序方向(提交给服务器)
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        down: function (rowParm)
        {
            ...
            var listdata = g._getParentChildren(rowdata);
            var index = $.inArray(rowdata, listdata);
            if (index == -1 || index == listdata.length - 1) return false;
            var selected = g.getSelected();
        ...
        //擴展方法,獲取列表全部數據 by uton
        getAllData: function (status, removeStatus)
        {
            var g = this, p = this.options;
            var data = [];
            //alert(JSON.stringify(p.data));
            $.each(p.data, function (index){
            	var o = p.data[index];
            	//if (o[p.statusName] == status || status == undefined)
                //{
                    data.push(g.formatRecord(o, removeStatus));
